...
public class vayra_forever_war extends BaseHullMod {
	...
	return (isForModSpec || Global.CODEX_TOOLTIP_MODE);
    }
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (isForModSpec || Global.CODEX_TOOLTIP_MODE) {
            ...
            tooltip.addPara("This ship is a legacy design stemming from the corrupted Eagle and Falcon MPCs originally captured by the Kadur Theocracy, built to conform to their aggressive Forever War fleet doctrine. ", opad);
            ...
            tooltip.addPara("• %s overload duration", pad, Misc.getPositiveHighlightColor(), "-"+String.valueOf(Math.round((int) ((1f - OVERLOAD_REDUCTION) * 100)))+"%");
            tooltip.addPara("• %s top speed", pad, Misc.getPositiveHighlightColor(), "+"+String.valueOf(Math.round(SPEED)));
            ...
            tooltip.addPara("• %s deceleration and turn handling", pad, Misc.getNegativeHighlightColor(), "-"+String.valueOf(Math.round(MANEUVERABILITY * 100f)) + "%");
