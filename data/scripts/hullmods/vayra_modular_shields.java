...
public class vayra_modular_shields extends BaseHullMod {
        ...
        if (ship.getHullSpec() != null) {
            switch (ship.getHullSpec().getHullId()) {
                ...
                default: {
                    ...
                    EXP2doEveryOtherPartStuff(ship, amount, engine);
    ...
    public static class EXP3doEveryOtherPartStuff implements DamageTakenModifier {
            ...
            public String modifyDamageTaken(Object param, CombatEntityAPI target, DamageAPI damage, Vector2f point, boolean shieldHit) {
                if (!shieldHit) damage.getModifier().modifyMult(SHIELD_GENERATOR_ID, 0.02f);
                return null;
            }
        }
    ...
    public static class EXPdoEveryOtherPartStuff implements DamageTakenModifier {
            ...
            public String modifyDamageTaken(Object param, CombatEntityAPI target, DamageAPI damage, Vector2f point, boolean shieldHit) {
                Color color;
                    ...
                    if (Global.getCombatEngine().getCustomData().get(STORED_GENERATORS_KEY) instanceof Map) {
                        storedGenerators = (Map<ShipAPI, ShipAPI>) Global.getCombatEngine().getCustomData().get(STORED_GENERATORS_KEY);
                    } else {
                    ...
                    if (generator != null && generator.isAlive() && !generator.getFluxTracker().isOverloaded()) {
                        ...
                        if (Global.getCombatEngine().getCustomData().get(JITTER_KEY) instanceof Map) {
                            jitters = (Map<ShipAPI, JitterData>) Global.getCombatEngine().getCustomData().get(JITTER_KEY);
                        } else {
                            jitters = new HashMap<>();
                        }
                        ...
                        if (jitters.containsKey(ship)) {
                            ...
                            jitter.age(Global.getCombatEngine().getElapsedInLastFrame());
                            if (jitter.timeToLive <= 0f) {
                                jitters.remove(ship);
                            }
                        }
                        damage.getModifier().modifyMult(SHIELD_GENERATOR_ID, generator.getFluxTracker().isOverloadedOrVenting() ? 1f : Math.max(0.1f, generator.getFluxTracker().getFluxLevel()*0.9f));
    ...
    private void EXP2doEveryOtherPartStuff(ShipAPI ship, float amount, CombatEngineAPI engine) {
        Color color;
