...
public class vayra_k2 extends BaseCommandPlugin {
	public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params, Map<String, MemoryAPI> memoryMap) {
		if (dialog == null) return false;
                ...
                ShipVariantAPI variant = Global.getSettings().getVariant("vayra_eagle_k_Hull").clone();
                variant.setSource(VariantSource.REFIT);
                variant.addPermaMod(HullMods.REINFORCEDHULL, true);
                variant.addPermaMod(HullMods.BLAST_DOORS, true);
                data.addShip(new ShipRecoverySpecial.PerShipData(variant, ShipRecoverySpecial.ShipCondition.PRISTINE, "KHS Babylonian Raven", "kadur_remnant", 0f));
